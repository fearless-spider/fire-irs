...
from utils import (INVALID_EMAILS, INVALID_PHONE_NUMS, INVALID_TINS,
                   INVALID_ZIPS, SCHEMA, TRANSMITTER_BLANK_MAP, VALID_ALL_DATA,
                   VALID_EMAILS, VALID_PHONE_NUMS, VALID_TINS, VALID_ZIPS,
                   check_invalid_email, check_invalid_phone_num,
                   check_invalid_tin, check_invalid_zip, check_valid_email,
                   check_valid_phone_num, check_valid_tin, check_valid_zip,
...
def test_transmitter_schema_overly_long_values():
    ...
    for key, value in temp["transmitter"].items():
        if isinstance(value, str):
            yield check_value_too_long, temp, ["transmitter", key], value + 99 * "A"


def test_transmitter_schema_phone_numbers():
    temp = deepcopy(VALID_TRANSMITTER)
    for num in VALID_PHONE_NUMS:
        yield check_valid_phone_num, temp, [
            "transmitter",
    ...
    for num in INVALID_PHONE_NUMS:
        yield check_invalid_phone_num, temp, [
            "transmitter",
            "contact_telephone_number_and_ext",
        ], num
        yield check_invalid_phone_num, temp, [
...
def test_transmitter_schema_validation_tins():
    ...
    for tin in VALID_TINS:
        yield check_valid_tin, temp, ["transmitter", "transmitter_tin"], tin
    for tin in INVALID_TINS:
        yield check_invalid_tin, temp, ["transmitter", "transmitter_tin"], tin


def test_transmitter_schema_emails():
    temp = deepcopy(VALID_TRANSMITTER)
    for email in VALID_EMAILS:
        yield check_valid_email, temp, ["transmitter", "contact_email_address"], email
    for email in INVALID_EMAILS:
        yield check_invalid_email, temp, ["transmitter", "contact_email_address"], email


def test_transmitter_schema_zip_codes():
    temp = deepcopy(VALID_TRANSMITTER)
    for zip_code in VALID_ZIPS:
        yield check_valid_zip, temp, ["transmitter", "company_zip_code"], zip_code
        yield check_valid_zip, temp, ["transmitter", "vendor_zip_code"], zip_code
    for zip_code in INVALID_ZIPS:
        yield check_invalid_zip, temp, ["transmitter", "company_zip_code"], zip_code
        yield check_invalid_zip, temp, ["transmitter", "vendor_zip_code"], zip_code
