...
from utils import (INVALID_PHONE_NUMS, INVALID_TINS, INVALID_ZIPS,
                   PAYER_BLANK_MAP, SCHEMA, VALID_ALL_DATA, VALID_PHONE_NUMS,
                   VALID_TINS, VALID_ZIPS, check_invalid_phone_num,
                   check_invalid_tin, check_invalid_zip, check_valid_phone_num,
                   check_valid_tin, check_valid_zip, check_value_too_long)
...
def test_payer_schema_overly_long_values():
    ...
    for key, value in temp["payer"].items():
        if isinstance(value, str):
            yield check_value_too_long, temp, ["payer", key], value + 99 * "A"
...
def test_payer_schema_validation_tins():
    ...
    for tin in VALID_TINS:
        yield check_valid_tin, temp, ["payer", "payer_tin"], tin
    for tin in INVALID_TINS:
        yield check_invalid_tin, temp, ["payer", "payer_tin"], tin


def test_payer_schema_zip_codes():
    temp = deepcopy(VALID_PAYER)
    for zip_code in VALID_ZIPS:
        yield check_valid_zip, temp, ["payer", "payer_zip_code"], zip_code
    for zip_code in INVALID_ZIPS:
        yield check_invalid_zip, temp, ["payer", "payer_zip_code"], zip_code
