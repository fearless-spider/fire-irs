...
VALID_ALL_PATH = "./tests/data/valid_all_MISC.json"
INVALID_PHONE_NUMS = [
    "+1 555 666 7777",
    "555 A44 B777",
    "123ABC5678",
...
VALID_TINS = ["12-1234567", "121234567", "12 1234567", "111-11-1111", "222 22 2222"]
INVALID_TINS = [
    "12-12345678",
    "12-123456",
    "12-12345-6",
...
PAYER_BLANK_MAP = [(7, 11), (44, 51), (240, 499), (508, 750)]

PAYEE_BLANK_MAP = [(45, 54), (328, 367), (408, 447), (499, 499), (508, 543), (723, 750)]

END_OF_PAYER_BLANK_MAP = [(10, 15), (304, 499), (508, 750)]
...
def dive_to_path(full_dictionary, path, value):
    def _dive_recursion(sub_dict, path, value):
        if len(path) > 1:
            sub_dict[path[0]] = _dive_recursion(sub_dict[path[0]], path[1:], value)
        if len(path) == 1:
...
def check_valid_phone_num(dict_obj, path, num):
    temp_obj = dive_to_path(dict_obj, path, num)
    assert (
        validate(temp_obj, SCHEMA) is None
...
@pytest.mark.xfail(jsonschema.exceptions.ValidationError)
...
def check_valid_email(dict_obj, path, email):
    ...
    assert validate(dict_obj, SCHEMA) is None, f"Email: {email}, Object: {dict_obj}"
...
def check_valid_zip(dict_obj, path, zip_code):
    ...
    assert (
        validate(dict_obj, SCHEMA) is None
